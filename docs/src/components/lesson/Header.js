...
class Header extends Component {
    ...
    beforeAppendChild(parent) {
        ...
        eventBus.subscribe('nextPrevVideo', ({ videos, id }) => {
            ...
            videos.map((v, i) => {
                if (v.videoId == id) {
                    data.next = (i == videos.length - 1) ? null : videos[i + 1].videoId
                    data.prev = (i == 0) ? null : videos[i - 1].videoId
                }
            })
            this.videos = videos
            this.data = data
    ...
    prev() {
        if (this.data.prev) {
            eventBus.dispatch('loadVideo', this.data.prev)
            eventBus.dispatch('nextPrevVideo', { videos: this.videos, id: this.data.prev })
        }
    }
    ...
    template({ }) {
        return `<header class="lesson__header">
        <nav>
            <div class="left">
